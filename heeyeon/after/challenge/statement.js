export function statement(invoice, plays) {
  function format(num) {
    return new Intl.NumberFormat('en-US', {
      style: 'currency',
      currency: 'USD',
      minimumFractionDigits: 2,
    }).format(num);
  }

  //? 적립 포인트
  function savePoint(perf) {
    let volumeCredits = 0;

    // 포인트를 적립한다.
    volumeCredits += Math.max(perf.audience - 30, 0);

    // 희극 관객 5명마다 추가 포인트를 제공한다.
    if ('comedy' === plays[perf.playID].type)
      volumeCredits += Math.floor(perf.audience / 5);

    return volumeCredits;
  }

  //? 총 적립 포인트
  function totalPoint() {
    return invoice.performances.reduce(
      (sum, perf) => (sum += savePoint(perf)),
      0,
    );
  }

  //? 연극 청구액
  function saveAmount(perf) {
    let thisAmount = 0;

    switch (plays[perf.playID].type) {
      case 'tragedy': // 비극
        thisAmount = 40000;
        if (perf.audience > 30) {
          thisAmount += 1000 * (perf.audience - 30);
        }
        break;
      case 'comedy': // 희극
        thisAmount = 30000 + 300 * perf.audience;
        if (perf.audience > 20) {
          thisAmount += 10000 + 500 * (perf.audience - 20);
        }
        break;
      default:
        throw new Error(`알 수 없는 장르: ${plays[perf.playID].type}`);
    }

    return thisAmount;
  }

  //? 총 청구액
  function totalAmount() {
    return invoice.performances.reduce(
      (sum, perf) => (sum += saveAmount(perf)),
      0,
    );
  }

  //? 내역서
  let result = `청구 내역 (고객명: ${invoice.customer})\n`;

  invoice.performances.forEach(
    perf =>
      (result += `  ${plays[perf.playID].name}: ${format(
        saveAmount(perf) / 100,
      )} (${perf.audience}석)\n`),
  );

  result += `총액: ${format(totalAmount() / 100)}\n`;
  result += `적립 포인트: ${totalPoint()}점\n`;

  return result;
}

// 사용예:
const playsJSON = {
  hamlet: { name: 'Hamlet', type: 'tragedy' },
  'as-like': { name: 'As You Like It', type: 'comedy' },
  othello: { name: 'Othello', type: 'tragedy' },
};

const invoicesJSON = [
  {
    customer: 'BigCo',
    performances: [
      {
        playID: 'hamlet',
        audience: 55,
      },
      {
        playID: 'as-like',
        audience: 35,
      },
      {
        playID: 'othello',
        audience: 40,
      },
    ],
  },
];

const result = statement(invoicesJSON[0], playsJSON);

const expected =
  '청구 내역 (고객명: BigCo)\n' +
  '  Hamlet: $650.00 (55석)\n' +
  '  As You Like It: $580.00 (35석)\n' +
  '  Othello: $500.00 (40석)\n' +
  '총액: $1,730.00\n' +
  '적립 포인트: 47점\n';

console.log(result);
console.log(result === expected);
